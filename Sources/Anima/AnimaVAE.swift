// AnimaVAE — decoder-only Swift/MLX port of the Qwen-Image / Wan-family 16-ch 3D-causal VAE.
//
// LIFTED verbatim from qwen-image-edit-swift/QwenVAE.swift (the parity-proven donor of the
// SAME VAE) and trimmed to the decode path Anima needs (post_quant_conv + decoder). The
// published `vae-bf16.safetensors` (xocialize/anima-mlx) is already canonical MLX-key layout
// (gamma 1-D, conv (O,kt,kh,kw,I), `.conv` wrapping) so loading is REMAP-FREE.
//
// Layout: channels-last (B,T,H,W,C) internally; one transpose at decode() entry/exit to the
// PT-facing (B,C,T,H,W). WanRMS = L2-over-channels * sqrt(C) * gamma, eps 1e-12.
...
public final class QwenCausalConv3d: Module {
    ...
    public init(inChannels: Int, outChannels: Int, kernel: (Int, Int, Int), padding: (Int, Int)) {
...
public final class WanResBlock: Module {
    ...
    public init(inChannels: Int, outChannels: Int) {
        ...
        self._convShortcut.wrappedValue = inChannels != outChannels
            ? QwenCausalConv3d(
                inChannels: inChannels, outChannels: outChannels, kernel: (1, 1, 1), padding: (0, 0))
            : nil
        super.init()
    }
...
/// Upsampler: nearest-2x then Conv2d. upsample3d variants also checkpoint a time_conv,
/// unused for single-frame (T=1) decode.
public final class WanUpsample: Module {
    ...
    public init(dim: Int, mode: String) {
        ...
        self._timeConv.wrappedValue = mode == "upsample3d"
            ? QwenCausalConv3d(inChannels: dim, outChannels: dim * 2, kernel: (3, 1, 1), padding: (1, 0))
            : nil
        super.init()
    }
...
public final class WanUpBlock: Module {
    ...
    public init(inChannels: Int, outChannels: Int, upsampleMode: String?) {
        ...
        self._upsamplers.wrappedValue = upsampleMode.map { [WanUpsample(dim: outChannels, mode: $0)] }
        super.init()
    }
...
public final class AnimaVAEDecoder: Module {
...
/// Decoder-only AutoencoderKLWan/QwenImage. `decode` takes/returns PT layout (B,16,T,H,W)/(B,3,T,8H,8W);
/// diffusers clamps the decoded output to [-1,1].
public final class AnimaVAE: Module {
    @ModuleInfo(key: "post_quant_conv") var postQuantConv: QwenCausalConv3d
    @ModuleInfo(key: "decoder") var decoder: AnimaVAEDecoder

    public var weightDtype: DType = .float32

    /// Wan21 latent format (from comfy latent_formats.Wan21 / vae config).
    ...
    public override init() {
        ...
        self._decoder.wrappedValue = AnimaVAEDecoder()
        super.init()
    }

    /// latents: (B,16,T,H,W) de-normalized -> image (B,3,T,8H,8W) in [-1,1].
    public func decode(_ latents: MLXArray) -> MLXArray {
        var x = latents.asType(weightDtype).transposed(0, 2, 3, 4, 1)  // -> (B,T,H,W,C)
        x = postQuantConv(x)
        x = decoder(x)
        x = clip(x, min: MLXArray(Float(-1)), max: MLXArray(Float(1)))  // diffusers decode() clamp
        return x.transposed(0, 4, 1, 2, 3)  // -> (B,C,T,H,W)
    }

    /// Wan21 denormalize: latents * std + mean (apply before decode).
